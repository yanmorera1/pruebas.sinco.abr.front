import {
  Alert,
  Typography,
  Card,
  Button,
  Input,
  Spinner,
  Option
} from '@material-tailwind/react'
import { BASE_URL, FORM_STATES } from '../consts'
import Layout from '../components/Layout'
import { useEffect, useState } from 'react'
import { useParams } from 'react-router-dom'
import { Link } from 'react-router-dom'
import { Select } from '@material-tailwind/react'

export default function UpdateSubject() {
  const [status, setStatus] = useState(FORM_STATES.INITIAL)
  const [subject, setSubject] = useState()
  const [teachers, setTeachers] = useState()
  const [selectedTeacherId, setSelectedTeacherId] = useState()
  const { subjectId } = useParams()

  useEffect(() => {
    getSubject(subjectId)
    getTeachers()
  }, [])

  const getSubject = (subjectId) => {
    fetch(`${BASE_URL}/subjects?subjectId=${subjectId}`, {
      method: 'GET',
      headers: new Headers({ 'content-type': 'application/json' })
    })
      .then((res) => {
        if (res.ok) return res.json()
        else throw new Error(res.statusText)
      })
      .then((json) => {
        setSubject(json[0])
      })
      .catch((error) => console.error(error))
  }

  const getTeachers = () => {
    fetch(`${BASE_URL}/teachers`, {
      method: 'GET',
      headers: new Headers({ 'content-type': 'application/json' })
    })
      .then((res) => {
        if (res.ok) return res.json()
        else throw new Error(res.statusText)
      })
      .then((json) => {
        setTeachers(json)
      })
      .catch((error) => console.error(error))
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    const model = Object.fromEntries(new FormData(e.currentTarget))
    model.teacherId = selectedTeacherId
    const values = Object.entries(model).map(([path, value]) => ({
      path,
      op: 'replace',
      value
    }))

    try {
      const response = await fetch(`${BASE_URL}/subjects/${subjectId}`, {
        method: 'PATCH',
        body: JSON.stringify(values),
        headers: new Headers({ 'content-type': 'application/json' })
      })
      if (response.ok) {
        setStatus(FORM_STATES.SUCCESS)
        e.target.reset()
      } else throw new Error(response.statusText)
    } catch (error) {
      console.error(error)
      setStatus(FORM_STATES.FAILED)
    }
  }
  if (!subject) return <Spinner />
  return (
    <Layout>
      <Card color='transparent' shadow={true} className='p-7'>
        <Typography variant='h4' color='blue-gray' className='mb-3'>
          Actualizar asingnatura - {subject.name} {subject.lastName}
        </Typography>

        <Link to={`/students`} className='font-medium text-blue-900'>
          Volver
        </Link>

        {status === FORM_STATES.SUCCESS && <Alert color='green'>Asingnatura actualizado</Alert>}
        {status === FORM_STATES.FAILED && <Alert color='red'>Error enviando los datos</Alert>}

        <form className='mt-8 mb-2' onSubmit={handleSubmit}>
          <div className='flex flex-col gap-5'>
            <Input label='Codigo' name='code' defaultValue={subject.code} required />
            <Input label='Nombre' name='name' defaultValue={subject.name} required />
            <Select label='Profesor' name='teacherId' defaultValue={subject.teacherId} required onChange={setSelectedTeacherId}>
              {teachers.map((teacher) => (
                <Option key={teacher.id} value={teacher.id}>
                  {teacher.name} {teacher.lastName}
                </Option>
              ))}
            </Select>
          </div>
          <Button className='mt-6' fullWidth type='submit'>
            Enviar
          </Button>
        </form>
      </Card>
    </Layout>
  )
}
